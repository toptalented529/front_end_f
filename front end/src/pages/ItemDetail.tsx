...
export const Item: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://127.0.0.1:8000/v1/upload/items"
        );
  ...
  return (
    <div className="">
      {loading ? (
        <div className="bg-gray-800 h-scren"></div>
      ) : (
        <div className="bg-gray-600   m-0 mx-0  flex flex-col items-center w-screen h-screen">
          <button
            className="bg-slate-800 absolute top-8 left-3 text-white font-bold rounded-md px-4 py -2"
            onClick={() => {
          ...
          {uploadView && (
            <UploadContainer
              imageNumber={items[Number(id)].imageId}
              itemNumber={Number(id)}
              onMessageChanges={handleMessage}
            />
          )}
          ...
          <button
            className="bg-slate-800 absolute top-8 right-32  text-white font-bold rounded-md px-4 py -2"
            onClick={() => {
